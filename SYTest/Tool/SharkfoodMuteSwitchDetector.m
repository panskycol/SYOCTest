...
@property(nonatomic,assign) BOOL shouldCheckVibrate;
...
-(void)didEnterBackground{
    if(self.shouldCheckVibrate){
        self.isPaused = YES;
        _isMute = YES;
        self.forceEmit = NO;
        if (self.silentNotify){
            self.silentNotify(self.isMute);
        }
    }
}
...
-(void) startCheckMute{
    self.shouldCheckVibrate = YES;
    self.isPaused = NO;
    self.forceEmit = YES;
    [self scheduleCall];
}
-(void) stopCheckMute{
    self.shouldCheckVibrate = NO;
    self.isPaused = YES;
}
-(void) setMuteSwitchDetectorPause:(BOOL) isPause{
...
-(void)complete{
    ...
    BOOL isMute = elapsed < 0.2; // Should have been 0.5 sec, but it seems to return much faster (0.3something)
